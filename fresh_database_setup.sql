-- Complete Fresh Setup for New Supabase Database
-- Run this in SQL Editor of your NEW Supabase instance

-- Create custom schema
CREATE SCHEMA hrmswishluv;
...
-- Create users table (base table, others depend on this)
CREATE TABLE hrmswishluv.users (
...
CREATE TABLE hrmswishluv.sessions (
  sid varchar PRIMARY KEY,
  sess jsonb NOT NULL,
  expire timestamp(6) NOT NULL
);
CREATE INDEX "IDX_session_expire" ON hrmswishluv.sessions(expire);

-- Create departments table
CREATE TABLE hrmswishluv.departments (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE hrmswishluv.designations (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    name text NOT NULL,
    description text,
...
CREATE TABLE hrmswishluv.attendance (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE hrmswishluv.leave_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES hrmswishluv.users(id),
...
CREATE TABLE hrmswishluv.expense_claims (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES hrmswishluv.users(id),
  title text NOT NULL,
...
CREATE TABLE hrmswishluv.employee_salary_structure (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE hrmswishluv.payroll (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE hrmswishluv.announcements (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  title text NOT NULL,
...
CREATE TABLE hrmswishluv.company_settings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE hrmswishluv.employee_profiles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL UNIQUE REFERENCES hrmswishluv.users(id),
  father_name text,
...
CREATE TABLE hrmswishluv.leave_assignments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES hrmswishluv.users(id),
  year integer NOT NULL,
...
-- Create permissive policies (allow all access for development)
